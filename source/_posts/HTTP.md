...
1. URI
2. 统一接口
3. 客户端获取的资源表述（json，xml，html）
4. 无状态 服务端不保存客户端状态：每一次请求中包含处理该请求所需的一切信息。 服务端不需要在请求间保留应用状态，只有在接受到实际请求的时候，服务端才会关注应用状态。  这种无状态通信原则，使得服务端和中介能够理解独立的请求和响应。  在多次请求中，同一客户端也不再需要依赖于同一服务器，方便实现高可扩展和高可用性的服务端。通过资源状态的变化进行改变。
5. 超链接 通过链接引导客户端：当你浏览Web网页时，从一个连接跳到一个页面，再从另一个连接跳到另外一个页面，就是利用了超媒体的概念：把一个个把资源链接起来.
6. 
...
-   **400 Bad Request**  ：请求报文中存在语法错误。参数不匹配
...
301与302：永久性重定向会将永久重定向记录缓存在浏览器中，在下次访问时直接跳转。
		 			临时重定向还是会先访问对应域名，之后再拿到重定向域名。
...
非对称密钥加密涉及到求幂与大数取模，且取模的质数也很大因此很慢
...
认证时会使用到CA的公钥，但对CA一样会有不信任，就可以再去上层CA确认，直到全球皆知的几个root CA，通过这种层层授信背书保证正常运转。

<div align="center"> <img src="https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/2017-06-11-ca.png" width=""/> </div><br>

#### 如何避免重放与篡改

设置Timestamp和随机数Nonce共同保证唯一，同样的请求只接受一次。

想篡改唯一键需要私钥签名，如果私钥签名对不上则视为无效。
