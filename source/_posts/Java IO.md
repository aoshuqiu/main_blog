...
![image-20220427221100958](Java IO\image-20220427221100958.png)
...
新的输入/输出 (NIO) 库是在 JDK 1.4 中引入的，弥补了原来的 I/O 的不足，提供了高速的、面向块的 I/O。BIO瓶颈为ServerSocket的accept与对输入流的read，一个请求一个线程。NIO通过通道防止阻塞ServerSocketChannel与SocketChannel，加入缓冲区。
...
I/O 包和 NIO 已经很好地集成了，java.io.\* 已经以 NIO 为基础重新实现了，所以现在它可以利用 NIO 的一些特性。例如，java.io.\* 包中的一些类包含以块的形式读写数据的方法，这使得即使在面向流的系统中，处理速度也会更快。如果有10000个socket但其中只有5个有事件，事件驱动，Reactor（事件监听）selector （多路复用选择器）
...
新建多路复用选择器Selector，将Channel与事件注册到selector中，让selector监听事件，select轮询调用select选择事件（这步阻塞）connect accept read/write，通过handler进行处理。

NIO 常常被叫做非阻塞 IO，主要是因为 NIO 在网络通信中的非阻塞特性被广泛使用。

NIO 实现了 IO 多路复用中的 Reactor 模型，一个线程 Thread 使用一个选择器 Selector 通过轮询的方式去监听多个通道 Channel 上的事件，从而让一个线程就可以处理多个事件。

通过配置监听的通道 Channel 为非阻塞，那么当 Channel 上的 IO 事件还未到达时，就不会进入阻塞状态一直等待，而是继续**轮询**其它 Channel，找到 IO 事件已经到达的 Channel 执行。
...
多Reactor IOReactor与ConnectReactor分离

accept与IO业务分别建立线程组处理，业务逻辑用线程池隔离

NioEventLoopGroup 用来开线程组，轮询使用线程处理事件

bootstrap 启动对象要绑定线程组与通道（通道是通过反射拿到NIOserverSocketchannel）option属性设置

服务器端：bootstrap绑定端口启动

客户端：bootstrap连接服务端，建立连接后服务器端会分配通道

客户端可以获得cs间通道，向channel中输出

编解码器

childhandler业务逻辑 handler通过上下文ctx获得channel

channelpipeline：双链表，出站入站方向相反

千万 亿级 ：服务器集群 netty单机峰值百万并发连接

zookeeper好处：监听通知

## java 流

![747f8c7c71d9e6448e72495acfcf52f7.png](https://img-blog.csdnimg.cn/img_convert/747f8c7c71d9e6448e72495acfcf52f7.png)

![6acad0487a9cf399f75709f1e0e206df.png](https://img-blog.csdnimg.cn/img_convert/6acad0487a9cf399f75709f1e0e206df.png)

一般就是file与buffer。

buffer会在流中存储一个byte数组或char数组，通过对它的存取进行流操作。
