...
![image-20220417173247983](Java 虚拟机\image-20220417173247983.png)
...
java的内存分为堆内内存和堆外内存，堆内内存是受JVM管控的，由JVM负责创建与回收；创建与回收都是自动进行的，不需要人为干预。

堆外内存和操作系统内存直接挂钩，不受jvm管制，由java进程直接向OS申请。

远程传输数据时要先将数据复制到堆外内存再发送，直接发到堆外内存会省去复制工作。
...
CMS缺点：因为需要保证用户正常使用内存，要使用标记清除算法，碎片化，可以设置在几次FULLGC之后执行一次整理。需要预留空间，发生错误就换成Serial old。并发标记时的浮动垃圾。
...
可以设置停顿时间。内存占用，region在被清空后可以充当其他角色
...
每个 Region 都有一个 Remembered Set，用来记录该 Region 对象的引用对象所在的 Region。通过使用 Remembered Set，在做可达性分析的时候就可以避免全堆扫描。以空间换时间。每次对新引用进行判断是否加入到Remembered Set中
...
对文件格式进行校验，例如所有二进制class文件开头都是cafebabe
...
声明类变量的类会在编译后生成clinit方法，对类变量进行初始化。
...
对于任意一个类，都需要由加载它的类加载器和这个类本身一同确立其在Java虚拟机中的唯一性。当我们比较两个类是否相等的时候，前提是：只有在这两个类是由同一个类加载器加载为前提下才有意义的。既然类可以通过不同加载器加载后，使其不与其他类equals。那么，是否可以自己写个java.lang.string类呢？我们都知道，自己写的类是appClassLoader加载到JVM的；jvm原生的string类是由bootstrapClassLoader加载的。这两个类是不同的加载器加载，是不eqs的。那么实际上可以吗？答案是：不可以。为什么不可以呢？因为双亲委派机制的缘故。
...
VisaulVM可以根据进程查看GC性能，eden s0 s1 old



默认初始化 显示初始化 构造器初始化 赋值

属性不存在多态性，方法才有多态性

字节码内部：JVM指令（字节码指令）不是机器码。操作码 操作数（对局部变量表进行操作）

如何查看：IDEA中jclasslab  javap -v

class文件结构： 、对于长度不定的会有一个计数器

​	魔数 cafebabe 校验是否合规classformat

​	class文件版本 与编译器版本有关，开发环境要与生产环境相同或向下兼容。

​	常量池 存放所有常量：字面量：文本字符串，声明为final的常量值

​											符号引用：类和接口的全限定名

​																字段的名称和描述符：描述符（类型（基本和引用），形参，返回值）

​																方法的名称和描述符

​																 解析时会将常量池中的符号引用转化为直接引用。

​	访问标志 public？abstract？final？

​	类索引，父类索引，接口索引集合  当前类，父类，实现的接口
...
2. 吞吐量：单位内完成工作量/GC中用户代码运行时间占总运行时间比例

3. 并发数：实际同时交互的请求数，1000人同时在线，估计并发数为5%-15%
...
线程控制，磁盘读写，数据库访问，网络I/O，垃圾收集

1.简单命令行工具：在toolsjar包中，有很多附赠的工具

​		1.jps java process Status：查看正在运行的java进程状态，会显示进程名与进程号-lmv hostid远程查看

​		2.jstat ：类装载，内存，垃圾收集，JIT编译（主要用于垃圾回收与内存泄漏）针对一个java进程而言。 可以每隔一段时间打印一次 -h打印表头

​			-option ： -class 类加载信息

​								JIT相关： -complier JIT中被编译的方法。

​								-gc 显示GC相关信息 比较启动时间与GC时间的比例超过20%说明压力大，超过90%随时可能OOM				OU 老年代使用内存比例不断上涨可能会存在内存泄漏问题。

​		3.jinfo 查看或调整虚拟机配置参数

​		![image-20220402142645238](Java 虚拟机\image-20220402142645238.png)

​	 4.jmap：到处内存映像文件&内存使用情况

​		-dump 生成转储快照 什么数据导致堆溢出 可以只转储存活对象，生成二进制文件，可以自动在OOM时进行转储

​		-heap 堆空间信息

​		-histo：堆空间对象统计信息

​	5.jhat：JVM Heap Analysis Tool 分析转储文件，可以在浏览器中查看 OQL语句

​	6.jstack： 打印线程快照。 可以看到死锁或死循环问题

​	7.jcmd：除了jstat都可以

2. 图形化工具
    	1. Visual VM
    	2. MAT 分析OOM时可以通过查看引用查看什么对象导致的
    
3. 内存泄漏

    1. 对象不再被程序用到了，但GC又不能回收 内存泄漏很可能导致内存溢出

    2. 内存泄漏原因：静态集合类，
    
       ​							单例模式，
    
       ​							外部类实例方法返回内部类实例对象，导致外部类不会被垃圾回收。
    
       ​							各种连接需要手动close，否则无法回收
    
       ​							变量作用域不合理，静态>成员变量>局部变量
    
       ​							改变存在哈希表中的对象的Hash值，导致之后删除不了
    
       ​							缓存使用weakHashmap 弱引用（在下一次GC回收）
    
       ​							监听回调
    
       3. 例如 在写一些类似Arraylist 数组+size结构时，pop时不要只更改size，也要将对应数组对象赋null
    
       ​							
    
### JIT

优势： 比解释执行速度快，编译器已经将字节码转化为机器指令缓存起来了，直接操作机器指令。

劣势：解释器可以立刻执行，响应速度快。

JIT会进行热点探测，将有价值的字节码编译成机器指令。解释器可以作为编译器后备。

热点：多次调用方法或方法体中的循环体，基于方法调用计数器的。热度衰减。
