...
- 如果获取锁失败，就到每个实例上释放锁。
- 红锁在redis服务器下线后要延时上线，防止由于空服务器上线导致多个业务获得同个锁。
- JVM stop the world 导致锁过期： 鸵鸟策略，zk+mysql 通过CAS版本号解决。
...
![image-20220606181439216](分布式\image-20220606181439216.png)
...
加入消息队列后如果库存系统无法访问，消息仍在消息队列中，可以先把消息持久化，等库存系统恢复后，就可以正常消费消息减去库存。
