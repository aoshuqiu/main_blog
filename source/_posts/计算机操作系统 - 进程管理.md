...
由于创建或撤销进程时，系统都要为之分配或回收资源，如内存空间、I/O 设备等，所付出的开销远大于创建或撤销线程时的开销。类似地，在进行进程切换时，涉及当前执行进程 CPU 环境的保存及新调度进程 CPU 环境的设置切换页目录以使用新的[地址空间](https://so.csdn.net/so/search?q=地址空间&spm=1001.2101.3001.7020)，而线程切换时只需保存和设置少量寄存器内容，开销很小。
...
关于一个条件的变量cv condition variables

wait(cv, mutex), 等待cv

signal/notify(cv) 唤醒一个等待cv的线程

broadcast/notifyAll(cv)	唤醒全部等待cv的线程，但唤醒全部也只有一个能拿到互斥锁的线程

生产者消费者问题不能用一个条件变量完成，而要用两个条件变量，保证消费者只唤醒生产者，生产者只唤醒消费者
